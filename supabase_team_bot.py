...
class TeamBotService:
    ...
    def get_user_collection_name(self, user_id: str) -> str:
        ...
        import hashlib
        user_hash = hashlib.sha256(f"{user_id}:{ENCRYPTION_KEY}".encode()).hexdigest()[:16]
    ...
    async def add_user_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text("Usage: /add_user <telegram_id or @username>")
            return
    ...
    async def remove_user_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text("Usage: /remove_user <telegram_id or @username>")
            return
    ...
    async def setup_bot(self):
        ...
        self.bot_app.add_handler(CommandHandler("remove_user", self.remove_user_command))
