...
class TelegramAuthService:
        ...
        try:
            ...
            if not await client.is_user_authorized():
                ...
                try:
                    ...
                    logger.info(f"[AUTH_SERVICE] Code type: {sent_code.type.__class__.__name__}")
                    ...
                    logger.error(f"[AUTH_SERVICE] Flood wait error: {e.seconds} seconds")
                    raise ValueError(f"Too many attempts. Please wait {e.seconds} seconds")
                    
            else:
                logger.warning(f"[AUTH_SERVICE] User already authorized for phone {phone}")
                raise ValueError("User already authorized")
                
        except Exception as e:
            logger.error(f"[AUTH_SERVICE] Error sending code: {type(e).__name__}: {str(e)}")
    ...
    async def verify_code(
        ...
        """
        logger.info(f"[AUTH_SERVICE] Verifying code for phone: {phone[:3]}...{phone[-2:]}")
        ...
        try:
            ...
            try:
                user_info = await client.sign_in(phone, code, phone_code_hash=phone_code_hash)
    ...
    async def check_session_validity(self, session_string: str) -> bool:
        ...
        try:
            ...
            logger.error(f"[AUTH_SERVICE] Session invalid: {type(e).__name__}: {str(e)}")
            return False