...
from telethon.tl.types import (
    Channel, Chat, User as TgUser, 
    Message as TgMessage, MessageMediaPhoto, MessageMediaDocument,
...
class TelethonChatService:
    ...
    async def get_user_chats(self, encrypted_session: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for dialog in dialogs:
                ...
                if isinstance(entity, Channel):
                    ...
                    logger.warning(f"[CHAT_SERVICE] Unknown entity type: {type(entity)}")
                    continue
                
                chat_info = {
                    "id": str(chat_id),
                    "title": entity.title if hasattr(entity, 'title') else f"{entity.first_name or ''} {entity.last_name or ''}".strip(),
                    "type": chat_type,
    ...
    async def index_chat(
        ...
        try:
            ...
            while True:
                ...
                logger.info(f"[CHAT_SERVICE] Processing batch of {len(messages)} messages")
                ...
                async with get_db() as db:
                    for msg in messages:
                        ...
                        if existing:
                            logger.debug(f"[CHAT_SERVICE] Message {msg.id} already indexed")
                            continue
            ...
            logger.error(f"[CHAT_SERVICE] Error indexing chat: {type(e).__name__}: {str(e)}")
            raise
        finally:
            await client.disconnect()
