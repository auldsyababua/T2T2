...
class SmartChunkingService:
    ...
    async def group_messages_for_chunking(
        ...
        for i, msg in enumerate(sorted_messages):
            ...
            if current_group:
                ...
                elif len(current_group_text) + len(msg.text or "") + 10 > self.max_chunk_size:
                    should_break = True
    ...
    async def _create_chunk_from_group(
        ...
        metadata = {
            ...
            "message_links": [
                f"https://t.me/c/{msg.chat_id}/{msg.msg_id}" 
                for msg in messages
            ],
    ...
    async def _create_chunk_from_message(
        ...
        if msg.reply_to_msg_id:
            ...
            replied_msg = next(
                (m for m in all_messages if m.msg_id == msg.reply_to_msg_id and m.chat_id == msg.chat_id),
                None
            )
            if replied_msg:
        ...
        if chunk_text.strip().endswith("?"):
            ...
            "yes", "no", "yeah", "nope", "yep", "done", 
            "fixed", "completed", "not yet", "will do"
        ]:
    ...
    async def _find_likely_question(
        ...
        for prev_msg in reversed(all_messages):
            ...
            if (prev_msg.date < msg.date and 
                prev_msg.sender_id != msg.sender_id and
                prev_msg.text and 
                prev_msg.text.strip().endswith("?")):
