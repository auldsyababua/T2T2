...
logger.info(
    f"[AUTH] Bot token loaded: {bool(BOT_TOKEN)}, last 4 chars: ...{BOT_TOKEN[-4:] if BOT_TOKEN else 'None'}"
)
...
async def telegram_webapp_auth(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    logger.info(
        f"[AUTH] Raw init_data from header: {repr(init_data)[:100] if init_data else 'None'}"
    )
    ...
    if not init_data:
        init_data = request.headers.get("x-telegram-init-data")
        logger.info(
            f"[AUTH] Trying lowercase header: {repr(init_data)[:100] if init_data else 'None'}"
        )
    ...
    if username and not is_user_authorized(username=username, user_id=telegram_id):
        ...
        raise HTTPException(
            status_code=403, detail="Not authorized to use this service"
        )
...
async def telegram_auth(
    ...
    if not auth_data.username:
        raise HTTPException(
            status_code=403, detail="You need a Telegram username to use this service"
        )

    if not is_user_authorized(auth_data.username):
        logger.warning(
            f"Unauthorized access attempt by @{auth_data.username} (ID: {auth_data.id})"
        )
        raise HTTPException(
            status_code=403,
...
async def get_current_user(
    ...
    try:
        ...
        logger.info(
            f"[AUTH] User authenticated: {user.username} (ID: {user.tg_user_id})"
        )
        return user
