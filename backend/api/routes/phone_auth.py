...
async def send_verification_code(
    request: SendCodeRequest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    """
    logger.info(
        f"[PHONE_AUTH] Send code request for phone: {request.phone_number[:3]}...{request.phone_number[-2:]}"
    )

    try:
        # Send code via Telethon
        phone_code_hash, session_string = await auth_service.send_code(
        ...
        return SendCodeResponse(
            phone_code_hash=phone_code_hash, session_string=session_string
        )
...
async def verify_code(request: VerifyCodeRequest, db: AsyncSession = Depends(get_db)):
    ...
    """
    logger.info(
        f"[PHONE_AUTH] Verify code request for phone: {request.phone_number[:3]}...{request.phone_number[-2:]}"
    )
    ...
    try:
        ...
        encrypted_session = session_service.encrypt_session(
            session_string, {"user_id": str(user.id), "phone": request.phone_number}
        )
        ...
        return VerifyCodeResponse(
            ...
            user={
                ...
                "phone_number": user.phone_number[:3]
                + "..."
                + user.phone_number[-2:],  # Masked
            },
        )
...
async def logout(
    ...
    try:
        if current_user.session_string:
            # Decrypt session
            decrypted_session = session_service.decrypt_session(
...
async def auth_status(
    ...
    return AuthStatusResponse(
        ...
        user={
            ...
            "phone_number": current_user.phone_number[:3]
            + "..."
            + current_user.phone_number[-2:]
