...
async def get_user_chats(
    ...
    try:
        ...
        logger.error(f"[TELETHON_TELEGRAM] Error getting chats: {type(e).__name__}: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def get_chat_history(
    ...
    logger.info(f"[TELETHON_TELEGRAM] User: {current_user.id}, limit: {limit}, offset: {offset_id}")
    ...
    try:
        ...
        logger.error(f"[TELETHON_TELEGRAM] Error getting history: {type(e).__name__}: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def get_indexing_status(
    ...
    if not status:
        logger.info(f"[TELETHON_TELEGRAM] No active indexing for user {current_user.id}")
...
async def index_chats_task(user_id: int, session_string: str, chat_ids: List[int]):
    ...
    try:
        for i, chat_id in enumerate(chat_ids):
            logger.info(f"[INDEXING_TASK] Processing chat {i+1}/{len(chat_ids)}: {chat_id}")
            ...
            try:
                ...
                async def progress_callback(progress_data):
                    logger.info(f"[INDEXING_TASK] Progress for chat {chat_id}: {progress_data['indexed']} messages")
                    
                    # Update status
                    chat_status["indexed_messages"] = progress_data["indexed"]
                    status["indexed_messages"] = sum(c["indexed_messages"] for c in status["chats"])
                    
                    # Calculate overall progress
                    completed = sum(1 for c in status["chats"] if c["status"] == "completed")
                    in_progress = sum(
                        c["progress"] for c in status["chats"] 
                        if c["status"] == "indexing"
                    )
                    status["overall_progress"] = ((completed + in_progress) / len(chat_ids)) * 100
                ...
                logger.info(f"[INDEXING_TASK] Completed chat {chat_id}: {result['total_messages']} messages")
                
            except Exception as e:
                logger.error(f"[INDEXING_TASK] Error indexing chat {chat_id}: {type(e).__name__}: {str(e)}")
        ...
        await cache.set(f"indexing_status_{user_id}", status, expire=3600)  # Keep for 1 hour
        
        logger.info(f"[INDEXING_TASK] Completed all chats for user {user_id}")
        logger.info(f"[INDEXING_TASK] Total messages indexed: {status['total_messages']}")
