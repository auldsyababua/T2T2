...
def get_stdlib_modules():
    ...
    return {
        ...
        'typing', 'enum', 'dataclasses', 'abc', 'asyncio', 'unittest',
        'logging', 'warnings', 'io', 'string', 'textwrap', 'hashlib',
        'hmac', 'secrets', 'base64', 'urllib', 'http', 'email',
        'contextlib', 'copy', 'pickle', 'sqlite3', 'csv', 'configparser',
...
def check_missing_imports():
    ...
    for imp in sorted(all_imports):
        imp_lower = imp.lower()
        if (imp_lower not in installed and 
            imp not in stdlib and 
            imp not in local_modules):
            missing.append(imp)
