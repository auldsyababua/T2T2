...
def verify_telegram_webapp_data(
    ...
    try:
        ...
        if calculated_hash != received_hash:
            logger.warning(
                f"[AUTH] Hash mismatch - calculated: {calculated_hash[:20]}..., received: {received_hash[:20]}..."
            )
        ...
        logger.error(
            f"[AUTH] Exception in verify_telegram_webapp_data: {type(e).__name__}: {str(e)}"
        )
        logger.error(
            f"[AUTH] init_data type: {type(init_data)}, value: {repr(init_data)[:100] if init_data else 'None'}"
        )
