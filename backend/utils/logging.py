...
def setup_logger(
    name: str, level: str = None, log_file: Optional[str] = None, console: bool = True
) -> logging.Logger:
    """
...
def log_api_response(
    logger: logging.Logger, status_code: int, duration_ms: float, **kwargs
):
    """Log API response details"""
    logger.info(
        f"API Response: {status_code} ({duration_ms:.2f}ms)",
        extra={"status_code": status_code, "duration_ms": duration_ms, **kwargs},
    )
...
def log_db_query(logger: logging.Logger, query: str, duration_ms: float, **kwargs):
    ...
    logger.debug(
        ...
        extra={"query": query, "duration_ms": duration_ms, **kwargs},
    )
...
def log_telegram_event(logger: logging.Logger, event_type: str, user_id: int, **kwargs):
    ...
    logger.info(
        f"Telegram Event: {event_type} for user {user_id}",
        extra={"event_type": event_type, "user_id": user_id, **kwargs},
    )


def log_embedding_operation(
    logger: logging.Logger, operation: str, count: int, duration_ms: float, **kwargs
):
    """Log embedding operations"""
    logger.info(
        f"Embedding {operation}: {count} items ({duration_ms:.2f}ms)",
        extra={
            "operation": operation,
            "count": count,
