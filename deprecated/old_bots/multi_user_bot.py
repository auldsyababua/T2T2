...
class UserSession:
    ...
    def add_bot_conversation(self, user_message: str, bot_response: str):
        ...
        with open(self.bot_history_file, "a") as f:
            json.dump(
                {"timestamp": timestamp, "user": user_message, "bot": bot_response}, f
            )
            f.write("\n")

        logger.info(f"[USER_{self.user_id}] Added conversation to memory")

    async def index_telegram_history(
        self, telethon_session: str, limit_per_chat: int = 1000
    ):
        ...
        for dialog in dialogs:
            ...
            logger.info(
                f"[USER_{self.user_id}]   Indexed {message_count} messages from {chat_title}"
            )

        # Add all documents to vector store
        if documents:
            logger.info(
                f"[USER_{self.user_id}] Adding {len(documents)} documents to vector store"
            )
...
class MultiUserBot:
    ...
    async def memory_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        answer, sources = await session.query(
            f"What do you know about user {user.first_name}?"
        )
        ...
        # Add source count
        bot_convos = sum(
            1 for s in sources if s.metadata.get("type") == "bot_conversation"
        )
        telegram_msgs = sum(
            1 for s in sources if s.metadata.get("type") == "telegram_message"
        )
