...
class OptimizedT2T2Bot:
    ...
    async def handle_any_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logger.info(
            f"File received from {user.username} ({user.id}) in {chat.title or 'DM'}"
        )
        ...
        try:
            ...
            s3_key = self._generate_s3_key(
                file_type, file_info["filename"], user.id, chat.id
            )
            ...
            await status_msg.edit_text(
                f"✅ {file_type.capitalize()} saved! Processing..."
            )
    ...
    def _generate_s3_key(
        self, file_type: str, filename: str, user_id: int, chat_id: int
    ) -> str:
        ...
        try:
            ...
            if not cached_result:
                ...
                if redis_client and results:
                    redis_client.setex(
                        cache_key, 300, json.dumps(results)  # 5 minute cache
                    )
    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if stats.data:
                ...
                await update.message.reply_text(
                    "No files stored yet. Send me some files!"
                )
